...
import { sendMessageAction, shareSessionAction, requestHumanHelp } from '@/app/actions/kogito-actions';
...
export default function ChatInterface({ sessionId, initialMessages }: { sessionId: string, initialMessages: Message[] }) {
  ...
  const handleEscalation = async () => {
      // In a real app, this would create a ticket or notify tutors
      if(confirm('Veux-tu demander de l\'aide à ton tuteur humain ? Il aura accès à cette conversation.')) {
          setEscalationRequested(true);
          
          try {
             // Server Action to Create Ticket/Notify
             const result = await requestHumanHelp(sessionId);
             ...
             if (result.success) {
                  if (result.action === 'REDIRECT' && result.url) {
                      setMessages(prev => [...prev, {
                          ...
                          role: 'system',
                          content: result.message || "Je te redirige..."
                      }]);
                      setTimeout(() => {
                  ...
                  setMessages(prev => [...prev, {
                      id: Date.now().toString(),
                      role: 'system',
                      content: '✅ Demande reçue ! Ton professeur principal a été notifié. Il te répondra bientôt (ou viendra te voir si tu es en classe).'
                  }]);
             } else {
           ...
           <button 
               ...
               className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold transition-all
                  ${escalationRequested 
                      ...
                      : 'bg-white/80 backdrop-blur border border-indigo-100 text-indigo-600 hover:bg-white hover:shadow-md'}
               `}
           >
               <User size={14} />
               {escalationRequested ? 'Demande envoyée' : 'Demander un humain'}
           </button>
           ...
           <button 
               ...
               className={`
                  ...
                  ${isPrivateMode 
                     ...
                     : 'bg-white/80 backdrop-blur text-slate-500 border-slate-200 hover:bg-white hover:text-slate-800 hover:shadow-md'}
               `}
           >
               {isPrivateMode ? (
                 <><Ghost size={14} /> <span className="hidden sm:inline">Mode Privé</span></>
               ) : (
                 <><EyeOff size={14} /> <span className="hidden sm:inline">Activer Vie Privée</span></>
               )}
           </button>
      </div>
              ...
              <div className={`flex max-w-[90%] md:max-w-[80%] gap-3 ${m.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                ...
                `}>
                  ...
                  {m.role === 'assistant' && (
                       <button 
                         onClick={handleEscalation}
                         className="mt-2 text-xs text-indigo-400 hover:text-indigo-600 underline opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1"
                       >
                           <User size={12} />
                           Je ne comprends pas, parler à un humain
                       </button>
                  )}
                </div>
              </div>
